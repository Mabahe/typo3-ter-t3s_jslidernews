...
class tx_t3sjslidernews_pi1 extends tslib_pibase {
  ...
  var $readmore = 0;
    ...
    if ( $this->extConf['ownMarker_1'] && (strpos($this->extConf['ownMarker_1'],'tt_news.')!==false) ) { 
      ...
      $this->extConf['ownMarker_1'] = substr($this->extConf['ownMarker_1'], 8);    
    }
...
/*    if ( mysql_num_rows($newsRes) == 0 && $this->conf['errorNews'] ) {
  		$newsConf['where'] = ' 1=1';
      $newsConf['where'] .= $_tt_news_obj->getLanguageWhere();
  		$newsConf['where'] .= '  AND tt_news.uid = '.$this->conf['errorNews'];
      $newsRes = $_tt_news_obj->cObj->exec_getQuery('tt_news', $newsConf);
    }
*/
    $newsRow = array();
  ...
  protected function getSliderContentRecords() {  
    ...
    if ( $this->extConf['ownMarker_1'] && (strpos($this->extConf['ownMarker_1'],'tt_content.')!==false) ) { 
      ...
      $this->extConf['ownMarker_1'] = substr($this->extConf['ownMarker_1'], 11);    
    }
    if ( $this->extConf['ownMarker_2'] && (strpos($this->extConf['ownMarker_2'],'tt_content.')!==false) ) { 
      $select .= ','.trim( $this->extConf['ownMarker_2'] );
      $this->extConf['ownMarker_1'] = substr($this->extConf['ownMarker_1'], 11);    
    }
    $this->conf['contentElement'] = $this->conf['contentElement'] ? $this->conf['contentElement'] : trim($this->conf['errorContent']); 
    ...
    if ( $this->extConf['ownMarker_1'] && (strpos($this->extConf['ownMarker_1'],'pages.')!==false) ) { 
      $select .= ','.trim( $this->extConf['ownMarker_1'] );
      $this->extConf['ownMarker_1'] = substr($this->extConf['ownMarker_1'], 6);    
    }
  ...
  protected function checkVideo( $mmFile ) {
    ...
    if ( $record['CType'] == 'media' || $record['tx_rgmediaimages_config'] ) {
      ...
      if ( $this->conf['requireImage'] == 1 && $this->conf['contentStyle'] == 'tt_news' ) $record['image'] = '';
      if ( $this->conf['onlyDefaultImage'] && $this->conf['contentStyle'] == 'tt_content' ) $record['image'] = '';
      $record['image'] = t3lib_div::trimExplode(',', $record['image']); 
